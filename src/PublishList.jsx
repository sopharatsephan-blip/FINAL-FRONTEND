...
export default function PublishList() {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        ...
        // เอาเฉพาะวิดีโอที่มี SummaryID แล้ว (สรุปเสร็จแล้ว) และเผยแพร่แล้วเท่านั้น
        const completed = data.filter((v) => v.SummaryID && v.VisibilityType === 'Public');
        setVideos(completed);
        setError('');
      } catch (err) {
  ...
  const handleDeleteClick = (e, video) => {
    e.stopPropagation(); // กันไม่ให้ trigger การคลิกที่การ์ด (ซึ่งจะ navigate ไปหน้า publish-summary)
    setVideoToDelete(video);
  };

  // ✅ กดยืนยันใน modal -> ลบจริงในฐานข้อมูล
  ...
  return (
    <div className="admin-purple-container">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '10px' }}>
          ...
          {!loading && !error && filtered.map((video) => (
            <div
              ...
              style={{ width: '100%', padding: '0', cursor: 'pointer', position: 'relative' }}
              ...
              <div style={{ padding: '24px 28px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    {/* ✅ ปุ่มถังขยะ - ลบวิดีโอ (สไตล์เดียวกับ EditList) */}
                    <button
      ...
      {/* ===== Modal ยืนยันการลบ (สไตล์เดียวกับ EditList) ===== */}
      {videoToDelete && (
        <div
